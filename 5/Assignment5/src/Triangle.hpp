...
public:
  MeshTriangle(const Vector3f *verts, const uint32_t *vertsIndex, const uint32_t &numTris,
    ...
    maxIndex += 1; // indicetes the number of points in the mesh (shared point is counted as 1)
    vertices = std::make_unique<Vector3f[]>(maxIndex);
    // same as: vertices = std::unique_ptr<Vector3f[]>(new Vector3f[maxIndex]);
    memcpy(vertices.get(), verts, sizeof(Vector3f) * maxIndex);
    vertexIndex = std::make_unique<uint32_t[]>(numTris * 3);
    memcpy(vertexIndex.get(), vertsIndex, sizeof(uint32_t) * numTris * 3);
    numTriangles  = numTris;
    stCoordinates = std::make_unique<Vector2f[]>(maxIndex);
