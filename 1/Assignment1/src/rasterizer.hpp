...
public:
  ...
  pos_buf_id load_positions(const std::vector<Eigen::Vector3d> &positions);
  ...
  void set_pixel(const Eigen::Vector3d &point, const Eigen::Vector3d &color);
...
private:
  void draw_line(Eigen::Vector3d begin, Eigen::Vector3d end);
...
private:
  ...
  std::map<int, std::vector<Eigen::Vector3d>> pos_buf;
