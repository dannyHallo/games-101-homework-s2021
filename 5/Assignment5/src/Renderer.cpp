...
// \param[out] *hitObject stores the pointer to the intersected object (used to retrieve material information,
// etc.) \param isShadowRay is it a shadow ray. We can return from the function sooner as soon as we have
// found a hit.
// [/comment]
...
void Renderer::Render(const Scene &scene, const std::string &projDir) {
  ...
  FILE *fp = fopen((projDir + "preview.ppm").c_str(), "wb");
