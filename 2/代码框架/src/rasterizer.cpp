...
static bool insideTriangle(int x, int y, const Vector3f *_v) {
  // TODO: Implement this function to check if the point (x, y) is inside the triangle represented
...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer,
  ...
  float f1 = (50.f - 0.1f) / 2.f;
  float f2 = (50.f + 0.1f) / 2.f;
    ...
    for (int y = 0; y < height; ++y) {
      if (insideTriangle(x, y, t.v)) {
        auto [alpha, beta, gamma] =
            computeBarycentric2D(static_cast<float>(x), static_cast<float>(y), t.v);
        ...
        set_pixel(Vector3f{static_cast<float>(x), static_cast<float>(y), z_interpolated}, t.getColor());
      }
    }
  }
}
